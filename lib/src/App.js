...
class App {
    ...
    buildRequestUrl(plugin_id) {
        if (!this.props.nowInstallInstance || !plugin_id)
            throw new Error(App_types_1.Errors.INCORRECT_CONFIG);
        return `https://${this.props.nowInstallInstance}.service-now.com/api/sn_cicd/plugin/${encodeURIComponent(plugin_id)}/activate`;
    }
    /**
     * Get plugin id
